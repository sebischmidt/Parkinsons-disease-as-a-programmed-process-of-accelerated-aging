---
title: "Processing methylation data - GSE203332"
...
df_manifest <- fread(manifest_file, sep = "\t", header = TRUE, na.strings = c("NA", "", "N/A", ".")) %>%
  ...
  dplyr::select(Probe_ID, chr, start, end, type, channel, target, nextBase, 
         address_A, address_B, # Keep some key manifest columns
         mapFlag_A, mapChrm_A, mapPos_A, mapQ_A, # Example mapping quality info
...
#anno <- read.csv(file.path(tableDir, paste0(sampleSet,"_probe_annotation_GRCh38.csv")))
```

```{r}
rm(list=c("df_gene", "df_manifest", "df_mask"))
```
...
pheno <- pheno %>%
  transmute(
    ...
    # Rename and remove initial text prefixes
    Slide = str_remove(characteristics_ch1.1, "slide: "),
    Array = str_remove(characteristics_ch1.2, "slide_position: "),
    Replicate = str_remove(characteristics_ch1.3, "is_technical_replicate: "),
    ReplicateNr = str_remove(characteristics_ch1.4, "duplicate_pair: "),
    Gender = str_remove(characteristics_ch1.6, "Sex: "),
    
    # Rename, clean, and convert to numeric
    Age = as.numeric(str_remove(characteristics_ch1.7, "age_at_death_years: ")),
    pmd = as.numeric(str_remove(characteristics_ch1.8, "pmd_min: ")),
    DiseaseDuration = as.numeric(str_remove(characteristics_ch1.10, "disease_duration_years: ")),
    
    # Rename and clean diagnosis columns
    ConditionClinical = str_remove(characteristics_ch1.9, "clinical_diagnosis: "),
    Condition = str_remove(characteristics_ch1.12, "neuropathological_diagnosis: "),
    
    # Create ID column
    ID = paste(Sample_Name, Slide, Array, sep = "_")
  )
...
```{r}
pheno$Basename = file.path(idatDir, pheno$ID)
...
# 4. Find ALL highly correlated pairs (Correlation > 0.65)
highly_correlated_pairs <- as.data.frame(as.table(snpCor), stringsAsFactors = FALSE)
colnames(highly_correlated_pairs) <- c("Sample1", "Sample2", "Correlation")
highly_correlated_pairs <- highly_correlated_pairs %>%
  filter(!is.na(Correlation) & Correlation > 0.65)

# 5. Check if the high correlation is due to an EXPECTED replicate pair
if (nrow(highly_correlated_pairs) > 0) {
  # Add the ReplicateNr information for both samples in each pair
  highly_correlated_pairs <- highly_correlated_pairs %>%
    left_join(pheno %>% dplyr::select(ID, ReplicateNr), by = c("Sample1" = "ID")) %>%
    rename(ReplicateNr1 = ReplicateNr) %>%
    left_join(pheno %>% dplyr::select(ID, ReplicateNr), by = c("Sample2" = "ID")) %>%
    rename(ReplicateNr2 = ReplicateNr)
  
  # Identify UNEXPECTED pairs:
  # - Where ReplicateNr is different between the two samples, OR
  # - Where ReplicateNr is NA for one or both samples
  unexpected_pairs <- highly_correlated_pairs %>%
    filter(is.na(ReplicateNr1) | is.na(ReplicateNr2) | ReplicateNr1 != ReplicateNr2)

  BAD.geneticrelationship <- unique(c(unexpected_pairs$Sample1, unexpected_pairs$Sample2))
  
} else {
  # If no pairs have correlation > 0.65, then there are no issues to report.
  BAD.geneticrelationship <- character(0) 
...
```{r}
rm(list = c("snp_betas", "snpCor", "corMax", "highly_correlated_pairs", "unexpected_pairs"))
```
...
```{r}
# 1. Align pheno and bVals/mVals
pheno <- pheno[pheno$ID %in% intersect(pheno$ID, colnames(bVals)), ]
pheno <- pheno[match(colnames(bVals), pheno$ID), ]


# 2. Create a list grouping replicate sample IDs
all_replicates <- split(pheno[!is.na(pheno$ReplicateNr), ]$ID, 
                        pheno[!is.na(pheno$ReplicateNr), ]$ReplicateNr)

# 3. Skip replicates with only one entry (after QC filtering)
replicates <- all_replicates[sapply(all_replicates, length) > 1]


# 4. Average the replicate samples
bVals_replicates_avg <- sapply(replicates, function(ids) rowMeans(bVals[, ids], na.rm = TRUE))
mVals_replicates_avg <- sapply(replicates, function(ids) rowMeans(mVals[, ids], na.rm = TRUE))

new_ids <- sapply(replicates, `[`, 1)
colnames(bVals_replicates_avg) <- new_ids
colnames(mVals_replicates_avg) <- new_ids

# 5. Combine the unique samples with the new averaged samples
replicates <- unique(unlist(replicates))
bVals <- cbind(bVals[, setdiff(colnames(bVals), replicates), drop = FALSE],
                    bVals_replicates_avg)
mVals <- cbind(mVals[, setdiff(colnames(mVals), replicates), drop = FALSE],
                    mVals_replicates_avg)
...
### Neuron/glia proportions

```{r}
library(HiBED)
cellCounts <- HiBED_deconvolution(bVals, h=1) 
cellCounts$NeuronGliaProp <- (cellCounts$Neuronal/cellCounts$Glial)
cellCounts$ID <- rownames(cellCounts)

pheno <- pheno %>%
  left_join(cellCounts, by = "ID")

#cellCounts_backup <- cellCounts
```

```{r}
cell_type_cols <- c("Neuronal", "Glial", "Endothelial and Stromal")

# Reshape data from wide to long format for ggplot
pheno_long_for_plot <- pheno %>%
  dplyr::select(Condition, all_of(cell_type_cols)) %>%
  pivot_longer(cols = -Condition, names_to = "CellType", values_to = "Proportion")

# Create the boxplot
ggplot(pheno_long_for_plot, aes(x = CellType, y = Proportion, fill = Condition)) +
...
# Visualize Neuron/Glia Ratio
ggplot(pheno, aes(x = Condition, y = NeuronGliaProp, fill = Condition)) +
  geom_boxplot(show.legend = FALSE, alpha = 0.7, outlier.shape = NA) + 
  geom_jitter(width = 0.15, alpha = 0.6, height = 0, show.legend = FALSE) + 
  labs(
    x = "",
    y = "Neuron / Glia Ratio"
  ) +
...
rm(list = c("cell_type_cols", "pheno_long_for_plot", "cellCounts"))
#cellCounts <- read.csv(file.path(tableDir, paste0(sampleSet, "_cell_counts.csv")),  row.names = 1)
```
...
```{r}
covariates <- c("Age", "Gender", "NeuronGliaProp", "pmd")
```
...
```{r}
rm(list = c("bVals", "mVals", "mVals_harmonized", "resid_bVals", "fit", "fit_beta"))
```
...
```{r}
# Subset to interesting comparisons
pheno_harmonized <- pheno_harmonized %>%
  filter(Condition %in% c("CONTR", "PD", "PDD"))

pheno_harmonized$Condition <- factor(pheno_harmonized$Condition, levels = c("CONTR", "PD", "PDD"))
levels(pheno_harmonized$Condition) <- make.names(levels(pheno_harmonized$Condition))

resid_mVals <- resid_mVals[, pheno_harmonized$ID]
bVals_harmonized <- bVals_harmonized[, pheno_harmonized$ID]
...
```
```{r}
results_PDD <- diff_pipeline(
  comparison_name = "Ctrl_PDD",
  sampleSet = sampleSet,
  pheno_full = pheno_harmonized,
  bvals_full = bVals_harmonized, 
  mvals_full = resid_mVals,     
...
```{r}
results_PD <- diff_pipeline(
  comparison_name = "Ctrl_CombinedPD-PDD",
  sampleSet = sampleSet,
  pheno_full = pheno_harmonized,
  bvals_full = bVals_harmonized, 
  mvals_full = resid_mVals,     
